...
router.post("/post/:idCandidat", AuthorizationMiddleware, async (req, res) => {
    try {
        ...
        const educationNew = new ProjectModel(req.body);
...
router.put("/edit/:idEducation", AuthorizationMiddleware, async (req, res) => {
    try {
        ...
        const educationExit = await ProjectModel.findByIdAndUpdate({ _id: idCandidat }, req.body);
...
router.get("/get_educations/candidat/:idCandidat", AuthorizationMiddleware, async (req, res) => {
    try {
        ...
        const educationList = await ProjectModel.find({ access: true, idPerson: idCandidat });
...
router.put("/hide_education/:idEducation", AuthorizationMiddleware, async (req, res) => {
    try {
        ...
        const educationExist = await ProjectModel.findById({ id: idEducation });
        if (!educationExist) {
