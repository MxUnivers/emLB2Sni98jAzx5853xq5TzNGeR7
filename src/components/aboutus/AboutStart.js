    ...
    <div class="max-w-7xl mx-auto px-6 md:px-12 xl:px-6">
        <div class="relative">
            ...
            <div class="mt-6 m-auto space-y-6 md:w-8/12 lg:w-7/12">
                <h1 class="text-center text-4xl font-bold text-gray-800 dark:text-white md:text-5xl">Rejoignez-nous dès aujourd{"'"}hui</h1>
                <p class="text-center text-xl text-gray-600 dark:text-gray-300">
                    Faites partie  de personnes qui utilisent Jouman pour des ses différents services dans la monde de l{"'"}entreprise et autres.
                </p>
                <div class="flex flex-wrap justify-center gap-6">
                    {
                        getAndCheckLocalStorage(localvalue.TYPEACCESS)==typePersonConnected[0] ?
                        <a href={`${routing.connexion_recuteur}`} class="relative flex h-12 w-full items-center justify-center px-8 before:absolute before:inset-0 before:rounded-full before:bg-primary before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95 sm:w-max">
                        <span class="relative text-base font-semibold text-white dark:text-dark">Commencer</span>
                    </a>:null
                    }
                    {
                        getAndCheckLocalStorage(localvalue.TYPEACCESS)==typePersonConnected[1] ?
                        <a href={`${routing.connexion}`} class="relative flex h-12 w-full items-center justify-center px-8 before:absolute before:inset-0 before:rounded-full before:bg-primary before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95 sm:w-max">
                        <span class="relative text-base font-semibold text-white dark:text-dark">Commencer</span>
                    </a>:null
                    }
                    {
                        /**<a href="/a-propos" class="relative flex h-12 w-full items-center justify-center px-8 before:absolute before:inset-0 before:rounded-full before:border before:border-transparent before:bg-primary/10 before:bg-gradient-to-b before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95 dark:before:border-gray-700 dark:before:bg-gray-800 sm:w-max">
